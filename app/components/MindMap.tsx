...
type IdeaNodeRenderData = IdeaNodeData & {
  ...
  onRemoveSubIdea?: (nodeId: string, index: number) => void;
};
...
function IdeaNode({ id, data, selected }: NodeProps<Node<IdeaNodeRenderData>>) {
  ...
  return (
    <div
      className="relative min-h-[68px] w-fit min-w-[200px] max-w-[280px] rounded-xl border bg-card px-3 py-3 pr-11 shadow-sm transition-shadow"
      style={{
          ...
          {data.subIdeas.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1">
              {data.subIdeas.slice(-4).map((item, index) => (
                ...
                >
                  <span className="max-w-[150px] break-words">{item}</span>
                  <button
                    type="button"
                    className="rounded-sm p-0.5 text-muted-foreground transition-colors hover:bg-background hover:text-foreground"
                    onClick={() => data.onRemoveSubIdea?.(id, index)}
                    aria-label={`Delete sub idea ${item}`}
                  >
                    <X className="size-3" />
                  </button>
                </span>
...
export default function MindMap() {
  ...
  const removeSubIdeaFromNode = useCallback(
    (nodeId: string, subIdeaIndex: number) => {
      setNodes((currentNodes) =>
        currentNodes.map((node) =>
          node.id === nodeId
            ? {
                ...
                data: {
                  ...node.data,
                  subIdeas: (node.data.subIdeas ?? []).filter((_, index) => index !== subIdeaIndex),
                },
              }
            : node,
        ),
    ...
    () =>
      nodes.map((node) => ({
        ...
        data: {
          ...
          onRemoveSubIdea: removeSubIdeaFromNode,
        },
      })),
    [nodes, addSubIdeaToNode, removeSubIdeaFromNode],
  );
        ...
        <div className="mt-3 space-y-4">
            ...
            {selectedNode.data.subIdeas.length > 0 ? (
              <div className="space-y-2">
                {selectedNode.data.subIdeas.map((subIdea, index) => (
                  <div
                    key={`${subIdea}-${index}`}
                    className="flex items-start justify-between gap-2 rounded-md border border-border bg-background px-2.5 py-2 text-xs"
                  >
                    <span className="min-w-0 flex-1 whitespace-normal break-words leading-relaxed text-foreground">
                      {subIdea}
                    </span>
                    <button
                      type="button"
                      className="shrink-0 rounded-sm p-1 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                      onClick={() => removeSubIdeaFromNode(selectedNode.id, index)}
                      aria-label={`Delete sub idea ${subIdea}`}
                    >
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              rows={3}
              className="min-h-[84px] whitespace-pre-wrap break-words"
            />
          </div>
